import ProForm, { ProFormDigit, ProFormSelect, ProFormSwitch, ProFormText, ProFormTimePicker, ProFormSlider } from "@ant-design/pro-form";
...
export const PropsEditor: FC<Props> = React.memo((componentProp) => {
    ...
    const positionProps = preCheck(Object.keys(componentProp.position).map(key => ({
        label: key,
        type: 'number',
    ...
    let posEditor = <>
        ...
        <ProForm
            submitter={false}
            layout="horizontal"
            onFieldsChange={onFieldsChange.bind(null, position, adaptors)}
        >
    ...
    let propEditor = <>
        ...
        <ProForm
            submitter={false}
            layout="horizontal"
            onFieldsChange={onFieldsChange.bind(null, props, adaptors)}
        >
...
const onFieldsChange = (props: PropsObject, adaptors: { [key: string]: Function }, fields: FieldData[]) => {
    fields.forEach(field => {
        if (!field.errors?.length) {
            ...
            console.log(`prop '${field.name}' changed from '${props[ref]}' to '${adaptors[ref] ? adaptors[ref](field.value) : field.value}'${adaptors[ref] ? ' via adaptor' : ''}`);
